...
- HLS プレイヤー（`hls.js` + `Plyr`）による再生、ABR（Auto/解像度選択）、ローディング表示、エラー表示
- HLS 生成 API（`/api/hls/master`, `/api/hls/media`, `/api/hls/segment`）で ffmpeg により音声/動画を結合して `.m3u8` 配信
...
> HLS 生成には `ffmpeg` が必要です（`PATH` 上で `ffmpeg` コマンドが実行可能であること）。
