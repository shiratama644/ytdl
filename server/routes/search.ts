...
const searchCache = new SimpleCache<VideoItem[]>(180); // 3分キャッシュ
const suggestCache = new SimpleCache<string[]>(300); // 5分キャッシュ
...
searchRouter.get("/suggest", async (c) => {
  ...
  if (cached) {
    return c.json<SuggestionResponse>({ query, suggestions: cached });
  }
