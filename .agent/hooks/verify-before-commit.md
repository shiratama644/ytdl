...
- **build**: クライアントの成果物は `out/`（**Nginx が配信** = D12）。
  ...
  - **メタデータ解決**: `youtubei.js` のクライアントが**プロセスで 1 個**（毎リクエストで生成していない）であること、
    **キャッシュ → YouTube 取得**の順で縮退し、O9（TTL 付きキャッシュ + single-flight + バックオフ）があることをコードで確認
    （実ネットワークの可否 = **V6**。CI では**オフライン生成**（`generate_session_locally: true`）を使う）。
