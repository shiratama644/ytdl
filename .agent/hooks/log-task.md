...
- summary = タスク内容の kebab-case（例: `setup-project-structure`）。
- 同日に複数タスクがある場合は summary で識別（同日内は summary 辞書順でソート）。
...
- ログの目的は「次回の自分（別セッション）が過去の判断経緯を追えること」。主観・推測も明記してよい（AGENTS.md §7.3）。
