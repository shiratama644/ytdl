...
本規約は ytdl の技術スタック・ディレクトリ構成・検証コマンドに基づいて記述しています。
§1〜§5 は汎用の作業規約（作業単位・開発ワークフロー・テスト品質保証・Git 運用・完了条件）を、
§6〜§8 は ytdl 固有の知見（環境・サンドボックス制約・実装ルール・エージェント記憶システム）を扱います。
...
- ドキュメントは種類別フォルダに配置し、必ず `docs/README.md` の目次を更新する:
  - `docs/planning/` — 計画書（`_TEMPLATE.md` 形式。視聴・ダウンロード・プレイヤー等の設計検討時に作成）
  - `docs/ops/` — デプロイ・CI 運用手順（実際にデプロイする際に必要になったら作成）
