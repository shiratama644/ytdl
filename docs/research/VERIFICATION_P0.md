...
https://raw.githubusercontent.com/shiratama644/ytdl/<セッション固定ブランチ>/verification/v1b-gas-test.gs
...
### V1-e(第 5 回・試行 1) — ⚠️ **HTTP 429(レート制限)で未取得(キット再実行へ)**

実行: ユーザー / 2026-09-14T13:01Z / 生データ: `verification/Verification-Results.md`(コミット `3dcfce9`)

- `httpStatus = 429` / `htmlLen = 1648`(レート制限ページ)/ 抽出 0 回 = **データ未取得**。
- **原因推定**: キット群の**累計 fetch(~13 回/1 時間・同じ Google DC IP ブロック)**:
  v1b(embed 1 + /player 4)+ v1c(watch 2)+ v1d(watch 2 + /player 3)+ v1e(watch 1)。
  **一時的なレート制限**(v1d が 50 分前に同環境で成功している = 恒久的な壁ではない)。
- **この 429 が P00-D 設計に直接効く(O9)**: 本番リゾラは必ず
  ① **429/5xx のリトライ + バックオフ** ② **解決結果のキャッシュ(CacheService・TTL)**
  ③ **同一動画の同時実行マージ(single-flight)** を持たなければならない
  (= ユーザーの毎リクエストで watch ページを再 fetch しない)。
- **v1e 試行 2 用キット更新**: 3 回まで自動リトライ(30s / 60s バックオフ)+ 各試行の
  status 記録 + 最終失敗時のエラーページ冒頭 300 文字記録。
  **ユーザー側での要件: 前回実行から 10 分以上の間隔 + ページのリロード禁止**
  (リロード 1 回 = fetch 1 回 = レート制限の自己招致)。

### V1-e(第 5 回・試行 2) — ⚠️ **キットが長待ち化したため再設計(v1f)**

実行: ユーザー / 2026-09-14 夜 / 「デプロイして URL を開いたが**どれだけ待っても表示されなかった**」

- **原因(私の設計ミス)**: リトライ内蔵版は 1 回の実行が **最大 90 秒以上**読み込み状態
  (フィードバックなし)。429 が深いと fetch 自体が応答しにくくなる(Google の DC IP ブロックへの
  ネットワークレベルの制限)ため、GAS の実行時間上限(6 分)に近づき、ユーザーからは
  「永遠に読み込み中」に見える。
- **対策 = v1f(再設計・構文+モックスモーク検証済み)**:
  1. **`?probe=1` = 即返りの自己チェック**(YouTube fetch 0 回 = いつでも安全)。
     = デプロイ最新版の確認(= O5 の運用 pattern を本キットにも採用)。
  2. **1 回の実行 = 1 回だけ fetch(数秒で必ず JSON 返り)**。キット内リトライ廃止。
  3. リトライは**外部で**: 429 の JSON が返ったら → 閉じて **10〜30 分待って 1 回だけ
     開き直す**(= 各自動が数秒)。
  4. `fetchMs`(fetch にかかったミリ秒)を記録 = ハング診断用に。
