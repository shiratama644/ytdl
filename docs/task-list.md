...
| V1-b | GAS 実環境でのストリーム解決(1080p 確認) | 同上 | 進行中 | **第 1 回(2026-09-13)失敗**(マーカーなし + playability ERROR)。**第 2 回(2026-09-14・コミット `c53eddb`)**: 試行 1 = 旧ファイルで setMimeType 例外 / 試行 2 = **全 client 失敗**(C1・C2 embedded = ERROR「この動画は再生できません」/ C3 WEB = UNPLAYABLE / C4 ANDROID = HTTP 400)。**version 陳腐化は排除**(最新 2.20260911.01.00 でも NG)。embed ページは 200/131KB 実取得可 = 制限は `/player` 単位 → **データセンター IP + PO token/visitorData 欠如の疑い**。→ **次 = v1c 待ち**(`/watch/` ページの `ytInitialPlayerResponse` 抽出が主経路。watch も NG なら = IP 壁判定 → リゾラ置き場のアーキ分岐でユーザーと合意)。**P00-D 着手ゲートは v1c に移行** |
