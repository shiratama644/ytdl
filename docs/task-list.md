...
| ID | タスク | 証跡 | 状態 | 進捗 / 証拠 |
|---|---|---|---|---|
| V1-a | youtubei.js バンドル + Node 初期化(WEB_EMBEDDED_PLAYER 受入) | [VERIFICATION_P0.md](research/VERIFICATION_P0.md) | 完了(**現行の前提**) | 2026-09-12 / v18.0.0・1.3MB バンドル・create() 到達(通信はサンドボックス遮断)。**2026-09-23 追記: v18.1.0 を Node 22 で直接 import し、ネットワークなしで `Innertube.create()` が成立することを実測**(HANDOVER §7.10) |
| V1-b | GAS 実環境でのストリーム解決(1080p 確認) | 同上 | **✅ 完了(第 1〜6 回 2026-09-13〜15)**/ **現行: 参照(実装しない = D11 改訂。O9 の知見は継承)** | 最終結論(v1f・コミット `6143012`): `/player` = GAS IP から不可(×3 ラウンド)/**`/watch/` ページ抽出 = 可行**(playability OK / 30 形式 / 1080p(137)+audio(140))/**ストリーム URL = 全 30 形式 `signatureCipher`(復号必須 = youtube-dlp 型 transform 逆変換)**。**Phase A リゾラ = watch 抽出 + signature decipherer + O9 で確定** → **2026-09-23 改訂: 現行のメタデータ取得 = youtubei.js(D11)。抽出・復号は実装しない(証跡として保存。O9 の知見のみ継承)**。経緯: 第 1 回失敗 → v1b 全 client 失敗(version 陳腐化排除)→ v1c 抽出バグ(watch ページ取得は OK)→ v1d **成功**→ v1e 429(=O9)→ v1e 試行 2 長待ち化(設計ミス)→ v1f **完了** |
...
| V6 | **youtubei.js / InnerTube の前提確認**(2026-09-23 追加・server-first で必須。**キット v2**) | [verification/README.md](../verification/README.md) / [VERIFICATION_P0.md §V6](research/VERIFICATION_P0.md) | ⏳ **未実施(ユーザー実行待ち)** | キット `verification/v6-metadata-check.mjs`(**v2**): ① probe(環境 + youtubei.js の解決 + yt-dlp の有無 = ネットワーク 0 回)② `--mode=innertube`(生 fetch 4 件 = watch ページの API キー / `/player` / `/search`)③ `--mode=youtubei`(検索・動画)/ `--steps=all`(チャンネル・プレイリスト・ホーム・トレンド・Live Chat)④ `--mode=ytdlp`(将来の DL 用の記録)。**手順 = `npm install youtubei.js` → probe → innertube → youtubei**(詳細 = [verification/README.md](../verification/README.md))。**配備予定マシンで実行** → `verification/v6-result.json` を送付 → [Verification-Results.md](../verification/Verification-Results.md) へ記録。**連続実行禁止**(前回から 10 分未満は中断) |

> **設計判断の現状(2026-09-23 方針転換後・正本 = [HANDOVER.md](HANDOVER.md) §4)**: **構成 = サーバー一本**(D10 = 自宅 **Proxmox(LXC/VM)** + **Docker Compose**(Nginx + Bun/Hono API)。**GAS 期を設けず、GAS 版は実装しない**)。**再生 = iframe 一本**(既定 = `youtubeeducation.com/embed/{id}`。公式 embed(`youtube-nocookie.com`)へ差し替え可能な形にする。直リンク再生は実装しない)。**メタデータ = youtubei.js(InnerTube クライアント)**(D11 改訂 = 現段階の YouTube クライアントは youtubei.js のみ。第三者 API 不使用 = D2 の「自前実装」を維持。**yt-dlp は将来の DL 担当 = D13 で、現段階では導入しない**)。**自前のページ抽出も実装しない**(証跡として保存)。**動画ダウンロード = 保留(実装対象外)** → 旧スコープの DL 系検証(V1 のストリーム URL 解決 / V2 の直接取得 = CORS / V3 の StreamSaver / DL 機構 = D3〜D5・D9)は**保留として保存**(証跡は残す)。**検索・チャンネル・プレイリスト・トレンドは V6 の結果次第**(未確定の間は実装しない)。signature decipherer は実装しない。**P00 の推奨順 = B → C → D → E → F**(B/C は V5・V6 の結果に依存しないため先行可。**ユーザー GO 待ち**)。詳細仕様 = [HANDOVER.md](HANDOVER.md) §8.3。
...
| P00-D | **メタデータ解決**(`apps/api` = **youtubei.js クライアントの生成と使い回し** + 検索 / 動画 / チャンネル / プレイリスト / トレンドの正規化 + **O9**: キャッシュ/リトライ+バックオフ/single-flight。**実機能は V6 の結果次第**) | 同上 | 未着手(**V6 完了 + ユーザー GO 待ち**) | **保留**(実装しない)= signature decipherer / ストリーム URL 解決 / 自前ページ抽出 / yt-dlp(将来の DL = D13)/ DL 機構(D3〜D5・D9)。仕様 = [HANDOVER.md](HANDOVER.md) §8.3 |
...
| DOC-3 | **server-first への計画再構成**(ユーザー指示 2026-09-23 =「最初からサーバーを立てて作る」)。GAS 期を廃止し、自宅 Proxmox + Docker Compose(Nginx + Bun/Hono) / メタデータ = yt-dlp 主 + ページ抽出フォールバック / 再生 = iframe(維持)/ DL = 保留(維持)へ全文書を整合(**当時の内容。DOC-4 で youtubei.js へ改訂**)。**V6 キット作成** | 完了(2026-09-23) | ① 新規 `verification/v6-metadata-check.mjs`(依存なし・probe と `--mode=all/video/page/search/trend/ytsearch`・10 分ガード・結果を `v6-result.json` に保存)② 改訂 = README.md / docs/HANDOVER.md(§1〜§14・付録)/ AGENTS.md(§0・§1・§3.1・§6.1〜§6.3・§6.5)/ planning/PHASE0_PLAN.md(§2〜§11・R13〜R15 新設・§10.9.1 検証リスト)/ task-list.md / skills(index・tech-stack・project-overview・sandbox-constraints)/ docs/README.md / planning/README.md / research/README.md / verification/README.md・Verification-Results.md ③ 不整合修正 = 簡体字の混入(8 箇所)・参照節番号(§10.8 → §10.9.1)④ コミット = 本タスクのコミット(`docs(DOC-3): ...`、push 済み)。検証 = §3.1 の整合性 grep(相対リンク切れ 0・GAS 現行参照 0) |
| DOC-4 | **メタデータ取得を youtubei.js へ変更**(ユーザー指示 2026-09-23 =「yt-dlp と youtubei.js を使うように」+ 補足「現段階の YouTube クライアントは youtubei.js のみ。yt-dlp は今後のダウンロード機能のとき」)。**D11 改訂 + D13 新設**・V6 キットを v2 へ更新 | 完了(2026-09-23) | ① 改訂 = [planning/PHASE0_PLAN.md](planning/PHASE0_PLAN.md)(§2〜§11・D11/D13・検証リスト V6)/ [HANDOVER.md](HANDOVER.md)(§1 概要・§2 目標・§4 判断表 D2/D11/D13・§5 アーキ図・§6 検証・§7.10 新設・§8.2・§8.3・§10〜§14・付録)/ [AGENTS.md](../AGENTS.md)(§0・§3.1・§6.1・§6.3・§6.5)/ README.md / skills(index・tech-stack・project-overview・sandbox-constraints)/ hooks(verify-before-commit)/ docs/README.md・planning/README.md・research/README.md ② V6 キット `verification/v6-metadata-check.mjs` を **v2**(probe / innertube / youtubei / ytdlp / all。10 分ガード・リクエスト間 4 秒)へ更新 ③ 手順更新 = [verification/README.md](../verification/README.md)(`npm install youtubei.js` + モード別手順)/ [research/VERIFICATION_P0.md](research/VERIFICATION_P0.md) §V6(V6-0〜V6-4)+ 保留注記の追記 ④ 保留文書への注記 = [research/DOWNLOAD_MECHANISM_RESEARCH.md](research/DOWNLOAD_MECHANISM_RESEARCH.md)(DL 再開時の担当 = yt-dlp)/ [research/SHIATUBE_DEEP_RESEARCH.md](research/SHIATUBE_DEEP_RESEARCH.md) ⑤ コミット = 本タスクのコミット(`docs(DOC-4): ...`、push 済み `arena/01a0c3bb-ytdl`)。検証 = キットの `node --check` + probe / `--mode=youtubei` / `--mode=innertube` の実行(サンドボックス)+ 整合性 grep(相対リンク切れ 0・yt-dlp の現行利用記述 0) |
