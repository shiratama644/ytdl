...
| V1-b | GAS 実環境でのストリーム解決(1080p 確認) | 同上 | 進行中 | **第 1 回(2026-09-13)失敗**(マーカーなし + playability ERROR)。**第 2 回(2026-09-14・コミット `c53eddb`)**: 試行 1 = 旧ファイルで setMimeType 例外 / 試行 2 = **全 client 失敗**(C1・C2 embedded = ERROR「この動画は再生できません」/ C3 WEB = UNPLAYABLE / C4 ANDROID = HTTP 400)。**version 陳腐化は排除**(最新 2.20260911.01.00 でも NG)。**第 3 回(v1c・2026-09-14)**: `/watch/` ページは **200/718KB で取得成功 + マーカー存在 + botCheck なし**(= IP 壁は確定せず)だが、**キット側の抽出バグ 2 点**(マーカー初回出現誤認 / API キー新形式非マッチ)でデータ未取得 → **次 = v1d**(抽出修正版・敵対的モック検証済み)。**P00-D 着手ゲートは v1d に移行**(全経路 dead の場合のみ = IP 壁判定 → リゾラ置き場のアーキ分岐でユーザーと合意) |
