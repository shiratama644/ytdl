...
└── research/            # 調査結果
    ...
    ├── VERIFICATION_P0.md         ★ 検証証跡(V1〜V6・設計への影響)
    └── DOWNLOAD_MECHANISM_RESEARCH.md  DL 機構調査(**保留**・再開時に参照)
```

`docs/` 外に関連するディレクトリ:
- [`../verification/`](../verification/README.md) = 検証キット + 生結果(README = 実行手順。**現行の必須キット = V5(ブラウザ = `v5-browser-iframe-test.html`)/ V6(サーバー = `v6-metadata-check.mjs`)**、生結果は 1 ファイル = 最新のみ)
...
| 2 | [`planning/PHASE0_PLAN.md`](planning/PHASE0_PLAN.md) | **全体アーキテクチャ(自宅サーバー + API + メタデータ)・スタック・§10.9.1 検証リスト・リスク** |
