...
- バイト中継は原則しない。例外(自宅サーバー phase のオプション relay)は §10.10 に明記してから行う
...
│  Nginx(https) ─▶ Bun/Node Hono API(yt-dlp subprocess, PO token, Range 可)       │
│                 ─▶ 静的配信(Next.js export)+ Service Worker(StreamSaver)        │
│                 ─▶(任意) googlevideo バイト relay(フィルタ回避・ヘッダ付与)      │
└──────────────────────────────────────────────────────────────────────────────────┘
```
...
| ダウンロード | **mp4-muxer**(h264+aac→mp4)/ **webm-muxer**(vp9・av1+opus→webm)/ **File System Access API**(第一) / StreamSaver(Phase B) | C6, §10.8 |
...
│   └── home/                 # (P5) Bun/Hono + yt-dlp + sw/ (StreamSaver) + nginx.conf
├── scripts/
...
- **追加機能**:
  ...
  3. **(オプション)googlevideo バイト relay**: 学校のフィルタが `googlevideo.com` 自体を遮断した場合、relay 経由でストリームを転送 + `httpHeaders`(C5)をサーバー側で付与。帯域はミニ PC 回線で実測してから有効化
  4. PO token / bot チェック対策(yt-dlp の solver。`WEB_EMBEDDED_PLAYER` を中心に)
